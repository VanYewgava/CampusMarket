...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ config('app.name', 'CampusMarket') }}</title>
                        ...
                        @if (Route::has('login'))
                            @auth
                                ...
                                <form method="POST" action="{{ route('logout') }}" class="inline">
                                    @csrf
                                    <button type="submit" class="text-sm font-medium text-red-500 hover:text-red-700 transition ml-2">
                                        Keluar
                                    </button>
                                </form>
                                @else
                ...
                @if (Route::has('login'))
                    @auth
                        ...
                        <form method="POST" action="{{ route('logout') }}" class="block w-full">
                            @csrf
                            <button type="submit" class="block w-full text-left text-base font-medium text-red-500 hover:text-red-700">
                                Keluar
                            </button>
                        </form>
                        @else
        ...
        <footer class="bg-white border-t border-gray-200 pt-12 pb-8 mt-auto">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="border-t border-gray-100 pt-8 text-center">
                    <p class="text-sm text-gray-400">&copy; {{ date('Y') }} Campus Market. Project PPL Mahasiswa.</p>
                </div>
            </div>
        </footer>
    </body>
